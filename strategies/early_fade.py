"""
Early Fade Strategy (formerly fee_extremes).

Core edge: when Polymarket price hits extremes (< 0.22 or > 0.78) in the first
90 seconds, it's often an overreaction. Spot hasn't moved enough to justify the
extreme, so we fade (bet against) it. Fees happen to be near-zero at extremes
(< 0.4% vs 1.56% at midpoint), which is a structural bonus but not the primary edge.

Two modes:
  - Fade: price extreme early in window, likely overreaction. Buy cheap contracts.
  - Confirm: price extreme AND spot confirms direction. (Rarely triggers.)
"""
...
class EarlyFadeStrategy(BaseStrategy):
    """
    Fades early-window price overreactions at extremes.
    Fees near-zero at extremes (0.41% vs 1.56% at midpoint) provide structural bonus.
    """

    name = "early_fade"

    def __init__(self, config: EarlyFadeConfig = None):
        self.config = config or EarlyFadeConfig()
