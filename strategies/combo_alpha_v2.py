"""
Combo Alpha V2 — Tighter CONFIRM filters + cross-asset momentum.

Builds on combo_alpha's dual CONFIRM/FADE mode (the best strategy at +$459)
with two improvements:

1. Graduated CONFIRM thresholds: early window (15-90s) needs 2+ signals,
   late window (90-270s) needs 3/3 signals agreeing. Reduces late-window
   losers that dilute win rate.

2. Cross-asset momentum boost (from early_fade_v2): if BTC/ETH/SOL are
   moving together, the signal is more reliable.

3. Contradiction filter: if any individual signal is strongly opposite
   to the composite direction, skip — reduces false confirms.
"""
...
class ComboAlphaV2Config:
    ...
    min_composite_strength: float = 0.08   # |composite| must exceed this (was 0.05)
...
class ComboAlphaV2Strategy(BaseStrategy):
    """
    combo_alpha with tighter CONFIRM filters and cross-asset momentum.
    Keeps the same dual CONFIRM/FADE structure but reduces late-window
    false confirms that dilute win rate.
    """

    name = "combo_alpha_v2"

    def __init__(self, config: ComboAlphaV2Config = None):
        self.config = config or ComboAlphaV2Config()
