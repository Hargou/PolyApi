...
class OrderBookImbalanceConfig:
    ...
    min_book_depth: int = 1             # lowered: most events are synthetic 1-level books

    # Timing
    min_elapsed_sec: int = 10           # earlier entry (match early_fade timing)
    max_elapsed_sec: int = 90           # focus on early window where fading works

    # Edge requirement
    min_edge_bps: float = 30.0          # lowered to match profitable strategies

    # Price extreme gatekeeper (same as early_fade)
...
class OrderBookImbalanceStrategy(BaseStrategy):
    ...
    def evaluate(self, state: MarketState) -> Signal:
        ...
        is_extreme = p_market < cfg.extreme_low or p_market > cfg.extreme_high
        if not is_extreme:
            return Signal("hold", 0, 0,
                          f"not extreme: {p_market:.2f}")
