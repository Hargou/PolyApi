...
class VolatilityRegimeConfig:
    ...
    # Vol estimation
    min_ticks: int = 5                      # lowered (we enter at 15s now, fewer ticks available)
    ...
    # Timing
    min_elapsed_sec: int = 15               # earlier (need some ticks but not 90s)
    max_elapsed_sec: int = 90               # focus on early window where fading works

    # Price extreme gatekeeper (only trade at extremes where fees are cheap)
