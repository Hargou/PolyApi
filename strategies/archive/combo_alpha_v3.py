"""
Combo Alpha V3 — Spot confirmation gate on FADE mode.

Based on: combo_alpha_v2 (combo_alpha_v2.py)
Changes from parent:
  - Added spot confirmation gate to FADE mode: skip fade when spot confirms
    the extreme direction (hypothesis: fading legitimate moves = losers,
    proven in binary_reversal which gained +$150 from this fix)
  - spot_confirm_min_bps = 5.0 (same threshold that worked for binary_reversal)
"""
...
class ComboAlphaV3Config:
    ...
    extreme_low: float = 0.22
    extreme_high: float = 0.78
    ...
    # NEW: Spot confirmation gate for FADE mode
    # Skip fade when spot confirms the extreme direction by at least this much
    spot_confirm_min_bps: float = 5.0

    # CONFIRM mode — graduated thresholds
    early_confirm_signals: int = 2     # 15-90s: need 2+ signals
    late_confirm_signals: int = 3      # 90-270s: need 3/3 signals
    late_confirm_cutoff_sec: int = 90  # boundary between early/late confirm
    min_composite_strength: float = 0.08   # |composite| must exceed this
    max_contradiction: float = 0.20    # if any signal opposes direction by this much, skip
...
class ComboAlphaV3Strategy(BaseStrategy):
    """
    combo_alpha_v2 + spot confirmation gate on FADE mode.
    When spot confirms the extreme direction, the extreme is likely a legitimate
    move — don't fade it. This filter turned binary_reversal from -$16 to +$134.
    """

    name = "combo_alpha_v3"

    def __init__(self, config: ComboAlphaV3Config = None):
        self.config = config or ComboAlphaV3Config()
    ...
    def evaluate(self, state: MarketState) -> Signal:
        ...
        if not (is_extreme_low or is_extreme_high):
            return Signal("hold", 0, 0,
                          f"price {p_market:.3f} not at extreme")
        ...
        # Graduated confirmation: early window needs 2+, late window needs 3/3
        ...
        if composite > cfg.min_composite_strength and signals_bullish >= required_confirms:
            # Check contradiction: if any signal is strongly bearish, skip
            ...
            # -- NEW: Spot confirmation gate for FADE --
            # If spot confirms the extreme direction, skip the fade
            if is_extreme_high and state.spot_return_bps > cfg.spot_confirm_min_bps:
                mode = None  # spot confirms high = legitimate move, don't fade
            elif is_extreme_low and state.spot_return_bps < -cfg.spot_confirm_min_bps:
                mode = None  # spot confirms low = legitimate move, don't fade
            else:
                mode = "FADE"
                direction = "fade"
        else:
